...
We denote that ```math Q^*(s,a)``` is the optimal action-value function of $Q(s,a)$. In this case, the state space is continuous so it is practically
...
[Successfully landing the Lunar Lander on the Moon's surface](videos/lunar_lander.mp4)

[Failing to land the Lunar Lander on the Moon's surface](videos/lunar_lander_failed_example.mp4)
