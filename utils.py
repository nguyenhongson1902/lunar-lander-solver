...
import imageio
import IPython # For Jupyter Notebook
...
@tf.function # to boost up the training time
def agent_learn(experiences, gamma, optimizer, q_network, target_q_network):
    """
    ...
    Args:
      ...
      q_network: The Q-network. Taking a state and result in Q values
      target_q_network: The target Q^-network. Using to generate target values y
    """
...
def create_q_and_target_networks(env):
    """
        Create Q-Network and target Q^-Network with the same architecture as the Q-Network.

        Args:
            env: Gym environment

        Returns:
            q_network (TensorFlow Sequential): The Q-Network
            target_q_network (TensorFlow Sequential): The target Q^-Network
    """
...
def save_plot_history(point_history, filepath, **kwargs):
    """
